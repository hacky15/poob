        ...
        while len(buf) >= chunk_bytes:
            ...
            for name, score in prediction.items():
                ...
                if s >= self._threshold:
                    detected = True
                    log.info("Wake word hit", model=name, score=f"{s:.3f}", user=user_id)
...
class DualPipelineProcessor:
    ...
    def __init__(
        ...
        self._wake_detector = WakeWordDetector(
            ...
            threshold=0.5,  # Official OWW pipeline model — trained with ACAV100M + augmentation
        )
    ...
    def process_audio_frame(
        ...
        # This is why the first "Hey Jarvis" works but subsequent ones don't —
        # the model receives no audio between utterances, so its feature pipeline
        # has a multi-second gap when the next speech starts.
        wake_hit = self._wake_detector.process_frame(user_id, pcm_16k)
        if wake_hit:
            # Store wake word hit regardless of speech state.
            # _pending_wake tracks hits during silence so the next utterance
            # inherits them. Decays after 3 seconds to avoid stale activations
            # from noise during playback.
            pipeline._pending_wake = True
            pipeline._pending_wake_time = time.monotonic()
            if pipeline.speech_started and not pipeline.is_active:
        ...
        if is_speech and not pipeline.speech_started:
            ...
            if pending and (time.monotonic() - pending_time) < 3.0:
                pipeline.is_active = True
    ...
    def _emit_utterance(self, user_id: int, pipeline: UserPipeline) -> None:
        ...
        # Layer 2: Text-based wake word detection from Deepgram transcript.
        # Catches "Hey Poob" when the audio model misses (~50% recall).
        # This is the standard production approach — layered detection.
        is_addressed = pipeline.is_active or self._text_wake_word_match(transcript)
