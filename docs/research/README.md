...
- [[wake-miss-salvage-rate-is-openwakeword-false-positives]] — a 294-count "Deepgram never delivered a transcript" failure signature looked like the largest issue in the whole log; reading the actual salvaged transcripts (Whisper/Gemini hallucinations like "the quick brown fox...") plus the wake-gate decision distribution (4,454 acoustic-only fires vs 138 clean dual-confirms, 32:1) shows it's dominated by OpenWakeWord false-positives the dual-gate is correctly rejecting, not lost real commands. Corrects the framing before it drove an unnecessary fix
