...
## Fix 3 — persona must not volunteer marketplace (2026-05-29 voice/chat audit)

A follow-up live audit of ~17h of voice + text logs (pulled from `docker logs` + `conversation_messages`) confirmed the post-Fix-1/2 state: **zero marketplace leakage** in casual or voice responses, routing correct (only explicit "add to my list" / "scan" hit `deal_assistant`), voice replies in-character ("You're desperate for some trite, ancient, forgotten Japanese novelty."). The operator nonetheless directed that the contract be made explicit: *"only mention deals or searching or marketplace if inquired about — only ever when."*

Hardened `_build_system_prompt` (base prompt, so it applies to every variant — casual + tool, voice + text): added a RULE ("you're a buddy hanging out, NOT a service desk — don't bring up deals/marketplace/shopping/listings/watchlists/searching unless asked; you CAN do it when asked, only ever bring it up when they do") and a NEVER bullet ("Volunteer deal/marketplace/shopping talk when nobody asked"). Test: `test_system_prompt_forbids_unsolicited_marketplace` asserts the constraint on all four variants. Cheap insurance against model drift and the standalone-`_BASE_PROMPT` path (which still frames Poob as a "deal-hunting assistant" — only used when AgentRunner runs primary, not as sub-agent).

## Voice/chat reliability findings (audit 2026-05-29, NOT fixed here — need decisions)

The same audit surfaced genuine voice-pipeline issues, none marketplace-related, none fixed in this incident:

- **`model2vec` missing → semantic addressee scoring dead — RESTORED 2026-05-29 (Fix 4).** `address_detector.py` does `from model2vec import StaticModel` (`minishlab/potion-base-8M`) but `model2vec` was **not in `pyproject.toml`**, so the `semantic_relevance` signal (weight 0.18 in `MultiSignalAddressDetector`) had effectively never been live, and the `"Model2Vec unavailable"` warning re-fired on every decision (the failure wasn't cached). Operator chose restore. Fix: added `model2vec>=0.3.0` to pyproject; pre-bake `potion-base-8M` (~30MB) in the Dockerfile (step 4c, soft-fail like the Kokoro bake); added an `_embed_load_failed` latch so a genuine absence stops retrying + re-warning every decision. Tests: `tests/unit/test_address_detector.py` (latch-on-failure, load-once-on-success, semantic-relevance-safe-when-disabled). Behavioral note: semantic relevance is a *positive* "should respond" signal, so this makes Poob marginally more willing to answer non-wake follow-ups that relate to its last reply — acceptable, and the threshold (0.60) plus the explicit "only volunteer marketplace when asked" rule (Fix 3) keep it bounded.
- **Wake-word reliability.** `"Wake word fired but Deepgram never delivered transcript"` (lost responses) and erratic wake latency (observed 325 ms – 7602 ms; 6–7 s detections trigger on stale audio). This is [[wake-word-v4-phonetic-neighbor-followup]] / [[voice-latency-optimization]] territory — a tuning/retraining effort, not a quick source fix.
- **`Deepgram stream disconnected`** for 3 users simultaneously (23:50) — STT stream drop; check reconnect handling.
- **`VOICE_MAX_DAVE_PROTOCOL_VERSION=0 ignored`** — env var set to 0 but code falls back to davey default 1 (per [[dave-version-zero-rejected-by-e2ee-required-guilds]]). Harmless (handled) but the env var should be removed from the homelab config.

## Follow-ups

- **Deal sub-agent markdown/emoji (Fix 2 — RESOLVED 2026-05-29, source fix).** The "Your wishlist is empty 🚀" and 6-point numbered-form ("Add shit then") replies come from a *different* path — `_handle_deal` returns the `gpt-oss-120b` deal sub-agent output **raw** when >500 chars or multiline ([poob.py:1197-1199](../../src/poob/brain/poob.py)). That passthrough is a **documented intentional invariant** ([[poobbrain-architecture]]: "data-heavy responses bypass personality wrap"), so it was NOT flipped. Fixed at the source instead (operator chose this over superseding the invariant): `_SUB_AGENT_PREAMBLE` in [agent/prompts.py](../../src/poob/agent/prompts.py) now has a FORMATTING block forbidding markdown tables/`**bold**`/headers/emoji, mandating plain one-item-per-line data, and banning trailing "Let me know…" helpdesk boilerplate. The data-bypass invariant is preserved (data is still shown, just plain). Tests: `tests/unit/test_agent_prompts.py` (3, assert the constraint + that "show the data" survives).
...
- **`_wrap_in_personality` fallback paths** ([poob.py ~1749-1754](../../src/poob/brain/poob.py)) — verified 2026-05-29: the *exception* path already logs `"Personality wrap failed"`, so wrap errors are visible. The only un-logged fallback is the rare empty-content case (`return resp.choices[0].message.content or deal_response`), which now returns the plain-text deal output (post-Fix-2) — acceptable, no change warranted. Closed: not a real gap.
