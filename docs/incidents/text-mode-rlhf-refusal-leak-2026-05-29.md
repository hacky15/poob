...
- **`model2vec` missing → semantic addressee scoring dead.** `address_detector.py:123` does `from model2vec import StaticModel` but `model2vec` is **not in `pyproject.toml`** — the import is try/except-guarded, so the `semantic_relevance` signal (weight 0.18 in `MultiSignalAddressDetector`) has effectively never been live. Warning `"Model2Vec unavailable, semantic scoring disabled"` re-fires on every decision (the failure isn't cached, so `_ensure_model` retries + re-warns each call). Decision needed: (a) restore — add the dep + bake the static model into the image; or (b) retire the dead path + cache the failure to stop the log spam. Note: restoring makes Poob *slightly more* likely to respond to non-wake follow-ups (semantic is a positive signal) — weigh against the operator's "only when addressed" intent.
