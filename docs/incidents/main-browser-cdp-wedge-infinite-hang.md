...
- **Reduce wedge frequency at the source — DONE (memory-pressure guard).** The chromium renderer leak degrades CDP (audit: ~5.6 GB across ~22 procs climbing to the 4 GiB cap). Surgical PID/PGID reaping was **rejected**: browser-use spawns chromium internally, so identifying "orphan" processes is a fragile heuristic that could SIGKILL the live browser — the exact outage we're preventing. Instead `PatrolScheduler._maybe_restart_on_memory_pressure()` reads the cgroup memory fraction between cycles and, above `patrol_memory_restart_pct` (default 0.85) after a warmup window, force-exits so a fresh process reclaims the leaked memory — the same reliable mechanism as the watchdog, with zero risk of killing the wrong process. This both prevents the OOM-kill and removes the memory-pressure that degrades CDP into the wedge. Tests: `TestMemoryPressureGuard`.
