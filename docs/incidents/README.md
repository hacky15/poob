...
- [[silent-control-action-hallucinated-on-content-lacking-evidence]] — an addressed check-in with real (but non-command) content, e.g. "Hey, Poob. You alive there?", got routed to a stale repeat of an earlier apply_effect call — silent by design, so the user heard nothing at all, twice in 20 minutes. Fixed: `_music_safety_net` vetoes any silent/stateful control action with zero supporting keywords anywhere in the message (2026-09-09, resolved, not yet deployed)
