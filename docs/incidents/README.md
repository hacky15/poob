...
- [[nvidia-cerebras-account-entitlement-gaps]] — NVIDIA and Cerebras rungs classified as "stale model id" were actually account-level: NVIDIA's key has zero working models (platform-wide EOL sweep + missing entitlement for the new lineup), Cerebras hits 402 Payment Required on every model. Also found and fixed a real bug: CerebrasProvider.is_available() said True even when auto-detection had just proven every model broken. Needs operator action (re-provision/billing), code-side gaps closed (2026-09-08, active — awaiting account fixes)
