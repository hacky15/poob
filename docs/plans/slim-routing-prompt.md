...
The "~4k-token routing prompt → slim to ~1k → Groq lasts 3× longer" claim was **wrong**. The system-prompt TEXT is only ~620–700 tokens. The ~4k/call figure (`Requested 4006` in the Groq 429 body) is the **full payload**: system text + the two tool JSON schemas (~900–1100 tok, which we KEEP) + history + context transcript. Realistic saving from this change: **~490–535 tokens off the routing system prompt** (~57–60% of the prompt text, ~25–35% of routing input). That is roughly **57 Groq turns/day instead of ~50 — not 150.** This change is therefore primarily an **accuracy + correctness-hygiene** win (focused classifier + closing real test gaps), with a modest latency/token benefit. It does **not** by itself make Groq-primary sustainable or kill the daily-cap cliff.
