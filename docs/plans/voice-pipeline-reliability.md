...
2. **Proactive Deepgram reconnect (next, needs safety-valve note).** On `_listen_loop` exit, schedule one reconnect via the existing connect lock; keep the 5 s throttle as the inter-attempt floor. Write a gotcha/decision documenting why proactive-but-throttled beats lazy. Reduces Issue 2 and Issue 3 together.
3. **Lost-transcript: fail loud, not silent (next).** On `_deferred_emit` timeout for an addressed utterance, optionally emit a short "didn't catch that" so the user knows to repeat, instead of dropping silently. Consider extending the wait modestly (e.g. 2.0 s) — measure first.
4. **Metric relabel (low).** Rename `latency_ms` → `speech_to_wake_ms` at the wake log; add a true per-frame inference timer if we want real detection latency.
