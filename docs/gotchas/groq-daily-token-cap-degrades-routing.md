...
- **Local floor (BLOCKED):** self-hosted Qwen3 (qwen3:4b/8b, already pulled) would be an unlimited terminal rung, BUT benchmarked at **50–120 s per call** on the current homelab box (CPU-class; qwen3:8b times out). Accurate but unusable for real-time routing. Off the table until the operator confirms/fixes a usable GPU + Ollama passthrough — then it's a sub-second, unlimited floor.
