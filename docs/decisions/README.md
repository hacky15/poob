...
- [[disable-dead-vision-and-fallback-model-rungs]] — six more dead/stale model rungs found chasing the routing-cascade audit: Groq's ENTIRE vision-model category is gone (disabled cleanly), the Scout last-resort routing rung + two Gemini ids replaced with live-smoke-tested models, NVIDIA/Cerebras left unreplaced pending account fixes. `ProviderCooldownRegistry` gained permanent-failure (402/403/404/410) classification so a guaranteed-dead rung stops being re-probed on every cascade traversal (2026-09-08, active)
- [[provider-cooldown-registry]] — extracted the LLM routing cascade's rate-limit circuit breaker into a standalone `ProviderCooldownRegistry`, shared between routing AND the STT cascade via one instance — closes the gap a same-day PR left by shipping an unconsulted cooldown dict. Configurable thresholds, generic `filter_active` over any keyed list. Supersedes [[provider-circuit-breaker]] (2026-09-08, active)
- [[music-text-replies-are-toob]] — music replies are Toob's in EVERY modality: text @mention play requests get the same one-sentence Toob venom in chat AND are spoken in Toob's voice in VC (sentinel-tagged transport, stripped by the agent handler; `speak_if_in_channel` grew persona dispatch). Supersedes the "text stays Poob" convention; `[SILENT]` control acks stay neutral; no Boob roll on text (2026-07-12, active)
- [[music-search-candidate-rerank]] — search re-ranks ytsearch5 candidates by title-overlap + duration-fit when the top hit looks implausible for a song query (the 31-min pottery-video failure); pass-1 hit always competes so no query dead-ends that played before; long-form-intent queries (mix/playlist/hour) exempt. Tunes [[ytdl-search-best-guess-fallback]] on the evidence it asked for (2026-07-11, active)
