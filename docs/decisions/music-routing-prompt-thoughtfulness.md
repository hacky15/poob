...
- **`_music_safety_net` kept as a backstop, not invested in.** It's the "play " hardcode the operator dislikes; the prompt fix shrinks its load. If post-deploy telemetry shows the LLM now routes reliably, retiring/tightening it is a clean follow-up. Not removed now — that would regress the 8 catches until the prompt proves out.
  ...
  **2026-07-21 update: this risk materialized in prod, exactly as predicted below.** "Do you think we should play hard or standard?" (a BTD6 difficulty-mode opinion question) was correctly declined by the ENTIRE LLM cascade, then the safety net's substring-only `play_signals` match reversed that correct decision and queued a random video — precisely the "'let's play a game' → play 'a game'" failure this note warned about. The telemetry this note's exit plan asked for arrived: the LLM cascade routes reliably (it got this exact case right), so the backstop got tightened, not the prompt. See [[play-question-misrouted-to-play-command]] for the fix (`_looks_like_non_music_play_usage` guard, checked before both play-related overrides).
